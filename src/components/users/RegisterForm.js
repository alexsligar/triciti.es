...
export class RegisterForm extends Component {
    ...
    validations = {
        'name': val => {
            ...
            if (!isLength(val, {min:2, max:30})) return 'Name must be between 2 and 30 characters';
        },
        'username': val => {
            if (!val) return 'Username required';
            if (!val.match(/^[A-Za-z0-9_.]+$/)) return 'Username must only contain letters, numbers, underscores and periods';
            if (!isLength(val, {min:3, max:30})) return 'Username must be between 3 and 30 characters';
        },
        'email': val => {
        ...
        'password': val => {
            ...
            if (!isLength(val, {min:8, max:30})) return 'Password must be between 8 and 30 characters';
            if (!val.match(/^(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,31}$/)) return 'Password must contain uppercase, lowercase, and number';
        },
        'passwordConfirmation': val => {
            if (!equals(val, this.state.fields.password)) return 'Password confirmation must match password';
        },
        'bio': val => {
            if (!isLength(val, {max:255})) return 'Bio must be 255 characters or less';
        }
    }
    ...
    render() {
        ...
        return (
            <Dimmer.Dimmable active={/*this.props.registerUserProcessing.toString()*/'false'}>
                {
                    this.props.registerUserError ?
                        <Message 
                            error
                            content={this.props.registerUserError}
                        />
                    : null
                }
                ...
                <Dimmer active={this.props.registerUserProcessing}>    
                    <Loader>
                        Welcome aboard. Processing sign up.
                    </Loader>
                </Dimmer>
            </Dimmer.Dimmable>
        );
