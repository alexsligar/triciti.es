...
import { handleUpdateTag, removeUpdateTagError } from '../../actions/tags/updateTag';
import { handleDeleteTag, removeDeleteTagError } from '../../actions/tags/deleteTag';
...
export class UpdateTag extends Component {
    ...
    handleInputChange = (e) => {
        ...
        if (_.some(this.props.tags, { title: e.target.value }) && e.target.value !== this.props.title) {
    ...
    cancelEditMode = () => {
        this.setState({ editMode: false, value: this.props.title, tagExists: false });
    }
    ...
    render() {
        ...
        const { title, updateProcessing, updateError, deleteProcessing, deleteError } = this.props;

        let inputAction = (
            <Button.Group>
                <Button onClick={this.cancelEditMode} icon><Icon name='times' color='red' /></Button>
                <Button.Or />
                <Button onClick={this.handleCheckClick} icon><Icon name='check' color='green' /></Button>
            </Button.Group>
        );
        if (updateProcessing) {
        ...
        let errorLabel = null;
        if (tagExists) {
            errorLabel = (<Label pointing='right' color='red' basic>Tag already exists</Label>);
        } else if (updateError) {
            errorLabel = (<Label pointing='right' color='red' basic>Error saving. Try again.</Label>);
        }
        ...
        return (
            <Fragment>
                ...
                <Confirm 
                    ...
                    content={deleteError ? 'Error deleting tag. Try again?' : 'Are you sure you want to delete this tag?'}
                />
            </Fragment>
        )
    }
...
const mapDispatchToProps = { handleUpdateTag, removeUpdateTagError, handleDeleteTag, removeDeleteTagError, };
