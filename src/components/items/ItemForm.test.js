...
describe('form validations', () => {
  ...
  it('should validate location', () => {
    ...
    locationInput.simulate('change', {
      target: { name: 'location', value: 'abc' },
    });
    expect(wrapper.state().fieldErrors.location).toBe(
      'Location must be between 10 and 60 characters'
    );
    const letters = 'abcde';
    locationInput.simulate('change', {
      target: { name: 'location', value: letters.repeat(13) },
    });
    expect(wrapper.state().fieldErrors.location).toBe(
      'Location must be between 10 and 60 characters'
    );
    locationInput.simulate('change', {
      target: { name: 'location', value: 'Howard Amon Park' },
    });
  ...
  it('should validate end_date', () => {
    ...
    const valid = wrapper
      .instance()
      .validations['end_date'](end_date, start_date);
    expect(valid).toBe('End date must be after start date');
  });
...
describe('handleSelectChange', () => {
  ...
  it('should reset the date fields if type is not event', () => {
    ...
    wrapper
      .instance()
      .handleSelectChange({}, { name: 'type', value: 'activity' });
...
describe('handleStartDateChange', () => {
  it('should set the state of fields, fieldErrors, and formErrors', () => {
    ...
    expect(wrapper.state().formErrors).toBe(true);
    expect(wrapper.state().fieldErrors.start_date).toBe(
